...
ERROR_REGISTRY: Dict[str, Tuple[str, int]] = {
    ...
    "import.task.noChaptersDetected": ("未能识别到有效章节，请检查TXT内容", 200),
    ...
    # ---- 向导流式 tracker 阶段码（issue #33 Category 3）----
    # 通用默认码把 stage 作为参数（zh 模板插入中文名，en 模板不含 stage 以避免中文
    # 泄漏）；显式文案站各占一个码。message 原文案字节不变（raw 通道保留）。
    "progress.wizard.assigningCareers": ("分配角色职业...", 200),
    "progress.wizard.autoCreatedCharacters": ("🎭 自动创建了 {{created}} 个角色: {{names}}", 200),
    "progress.wizard.autoCreatedOrganizations": ("🏛️ 自动创建了 {{created}} 个组织: {{names}}", 200),
    "progress.wizard.awaitingConfirmation": ("生成完成，等待用户确认...", 200),
    "progress.wizard.careersAssigned": ("已分配{{assigned}}个职业", 200),
    "progress.wizard.chaptersAutoCreated": ("已自动创建{{created}}个章节", 200),
    "progress.wizard.charactersBatch": ("生成第{{batch}}/{{total}}批角色 ({{batch_size}}个){{retry_suffix}}", 200),
    "progress.wizard.charactersBatchExhausted": ("批次{{batch}}在{{max_retries}}次重试后仍然失败", 200),
    "progress.wizard.charactersBatchGenerating": ("生成第{{batch}}/{{total}}批角色中{{retry_suffix}}", 200),
    "progress.wizard.charactersBatchMismatch": ("批次{{batch}}生成数量不正确: 期望{{expected}}个, 实际{{actual}}个", 200),
    "progress.wizard.charactersBatchRetry": ("批次{{batch}}生成数量不正确，正在重试", 200),
    "progress.wizard.checkingCharacters": ("🎭 校验角色信息...", 200),
    "progress.wizard.checkingOrganizations": ("🏛️ 校验组织信息...", 200),
    "progress.wizard.cleanedReferences": ("已清理{{cleaned}}个无效引用", 200),
    "progress.wizard.complete": ("{{stage}}生成完成!", 200),
    "progress.wizard.creatingChaptersOneToOne": ("一对一模式：自动创建章节...", 200),
    "progress.wizard.creatingMemberships": ("创建组织成员关系...", 200),
    "progress.wizard.creatingOrganizations": ("创建组织记录...", 200),
    "progress.wizard.creatingRelationships": ("创建角色关系...", 200),
    "progress.wizard.generating": ("生成{{stage}}中... ({{current_chars}}字符){{retry_suffix}}", 200),
    "progress.wizard.loadingCareers": ("加载职业体系...", 200),
    "progress.wizard.loadingCharacters": ("加载角色信息...", 200),
    "progress.wizard.loadingProject": ("加载项目信息...", 200),
    "progress.wizard.parsingAiResult": ("解析AI返回结果...", 200),
    "progress.wizard.parsingCareers": ("解析职业体系数据...", 200),
    "progress.wizard.parsingOutlines": ("解析大纲数据...", 200),
    "progress.wizard.parsingWorld": ("解析世界观数据...", 200),
    "progress.wizard.preparingOutlines": ("准备生成{{outline_count}}个大纲节点...", 200),
    "progress.wizard.preparingPrompt": ("准备AI提示词...", 200),
    "progress.wizard.regenerateWorld": ("开始重新生成世界观...", 200),
    "progress.wizard.regeneratingWorld": ("重新生成世界观{{retry_suffix}}", 200),
    "progress.wizard.savingCareers": ("保存职业数据...", 200),
    "progress.wizard.savingCharacters": ("保存角色到数据库...", 200),
    "progress.wizard.savingOutlines": ("保存大纲到数据库...", 200),
    "progress.wizard.savingWorld": ("保存世界观到数据库...", 200),
    "progress.wizard.skipChapterCreation": ("细化模式：跳过自动创建章节", 200),
    "progress.wizard.start": ("开始生成{{stage}}...", 200),
    "progress.wizard.validatingCharacters": ("验证角色数据...", 200),
    "progress.wizard.validatingProject": ("验证项目...", 200),
    ...
    "task.already_terminal": ("任务已是终态：{{status}}", 200),
