...
1. **SSE 丢码**：`AIService.generate_text_stream` 是异步生成器，`_require_model()` 的
   ...
   handler 硬编码 `internal.generation_failed` 后，未配置/不合格模型的用户在所有流式
   功能（章节生成、章内续写、大纲、wizard、skill 聊天）上都只看到一句通用失败，
   拿不到通往设置页的码。⇒ 本文件用**真实 SSE 路由**读事件流，断言 `error_code` 是
   `validation.*` 且 params 带模型名/下限。
...
外加结构性守卫：泛型流式收尾不得再硬编码通用码吞掉 ApiError。
...
# 这些文件的 `except Exception` 之前**没有** `except (HTTPException, ApiError)` 分支，
# 硬编码通用码就会吞掉 ApiError。careers/characters/organizations 有先行的 ApiError
# 分支，其泛型收尾保持原样（非缺陷），所以不在本守卫范围内。
STREAMING_FILES_WITHOUT_APIERROR_BRANCH = (
    "api/chapters.py",
    "api/skills.py",
    "api/wizard_stream.py",
    "api/outlines.py",
)


def test_no_generic_sse_sink_swallows_api_errors():
    """守卫：泛型收尾必须走 error_from_exception / sse_code_for_exception。

    两种拼法都要查：`tracker.error(..., error_code=...)` 与
    `SSEResponse.send_error(..., code=...)`——只查前者会在 skills.py 的旧写法上假绿。
    """
    offenders: List[str] = []
    needles = (
        'error_code="internal.generation_failed"',
        'code="internal.generation_failed"',
        "await tracker.error(str(",
    )
    for rel in STREAMING_FILES_WITHOUT_APIERROR_BRANCH:
        text = open(os.path.join(REPO_APP, rel), encoding="utf-8").read()
        for needle in needles:
            if needle in text:
                offenders.append(f"{rel}: 泛型收尾仍写死/丢码 -> {needle}")
